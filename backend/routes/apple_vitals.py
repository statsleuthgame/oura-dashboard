...
async def get_apple_respiratory(request: Request, days: int = Query(default=30, ge=0, le=2200)):
...
async def get_apple_spo2(request: Request, days: int = Query(default=30, ge=0, le=2200)):
    conn = _get_db(request)
